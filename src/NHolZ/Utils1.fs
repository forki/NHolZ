...
let rec type_tyvars ty =
    ...
    | Type_var _ -> [ty]
    | Type_comp (_,tys) -> unions' type_eq (map type_tyvars tys)
...
let mk_const (x,ty) =
    ...
    let theta = try1 (type_match ty0) ty
                        (func, "Type doesn't match generic type of constant")
    mk_iconst (x,theta)
...
let dest_bin tm =
    try
        let (ftm1,tm2) = dest_comb tm
        let (f,tm1) = dest_comb ftm1
        (f,tm1,tm2)
...
let dest_binder tm =
    try
        let (f,tm1) = dest_comb tm
        let (v,tm0) = dest_abs tm1
        (f,v,tm0)
...
let mk_eq (tm1,tm2) =
    let (ty1,ty2) = (type_of tm1, type_of tm2)
    let () = assert1 (type_eq ty1 ty2)
                     ("mk_eq","Arg 1 type not equal to Arg 2 type")
    let f = mk_iconst ("=", [(a_ty,ty1)])
    mk_bin (f,tm1,tm2)
...
let mk_imp (tm1,tm2) =
    let func = "mk_imp"
    let (ty1,ty2) = (type_of tm1, type_of tm2)
    let () = assert1 (is_bool_type ty1) (func,"Arg 1 not boolean")
    let () = assert1 (is_bool_type ty2) (func,"Arg 2 not boolean")
    let f = mk_gconst "==>" in
    mk_bin (f,tm1,tm2)
...
let mk_exists (v,tm0) =
    let func = "mk_exists"
    let ty = try1 var_type v (func,"Arg 1 not a variable")
    let ty0 = type_of tm0
    let () = assert1 (is_bool_type ty0) (func,"Arg 2 not boolean")
    let f = mk_iconst ("?", [(a_ty,ty)])
    mk_binder (f,v,tm0)
...
let rec alpha_eq0 theta0 tm1 tm2 =
    match (dest_term tm1, dest_term tm2) with
      (Term_var _, Term_var _)
         -> let tm1' = try  assoc tm1 theta0  with HolFail _ ->  tm1
            let tm2' = try  inv_assoc tm2 theta0  with HolFail _ ->  tm2
    ...
    | (Term_abs (v1,tm01), Term_abs (v2,tm02))
         -> let theta0' = (v1,v2)::theta0
            let (ty1,ty2) = (var_type v1, var_type v2)
...
let variant vs0 v =
    let func = "variant" in
    let (x,ty) = try1 dest_var v           (func,"Not a variable")
    let xs0 = try1 (map var_name) vs0      (func,"Non-var in avoidance list")
    let x1 = string_variant xs0 x in
    mk_var (x1,ty)

let rec cvariant_name xs0 x =
      let x1 = string_variant xs0 x
      if (is_const_name x1)
...
(* avoided by giving fresh names to any binding vars in the original term     *)
(* that would otherwise clash with any new free vars that get inserted within *)
(* their scope.  To identify where variable capture would occur, the 'vs0'    *)
(* list of surrounding binding vars is passed down through subterms.  If a    *)
(* clash is spotted during the instn of a var, then a 'Clash' exception is    *)
(* raised and passed back up until caught at the lambda abstraction that      *)
...
let rec var_inst0 vs0 theta tm =
  match (dest_term tm) with
    Term_var _
       -> (
            try
                let tm' = assoc tm theta      (* HolFail if var not in 'theta' *)
                let vsf' = free_vars tm'
                if (disjoint' term_eq vsf' vs0)
                  then (* Free var instantiation doesn't clash *)
                       ...
                       let v_ = find (fun v0 -> mem' term_eq v0 vsf') (rev vs0)
  ...
  | Term_const _
       -> (* Constant *)
          tm
  | Term_comb (tm1,tm2)
       -> let tm1' = var_inst0 vs0 theta tm1
          let tm2' = var_inst0 vs0 theta tm2
          (* Function application with no clash in subterms *)
          if (tm1' = tm1) && (tm2' = tm2)  then tm  else mk_comb (tm1',tm2') //if something goes wrong consider that I've changed tm' == tm and tm2' == tm2
  | Term_abs (v,tm0)
       -> let theta' = fst_filter (fun v1 -> not (term_eq v1 v)) theta
          (try
             let tm0' = var_inst0 (v::vs0) theta' tm0
             (* Lambda abstraction with no clash in subterm *)
             if (tm0' = tm0)  then tm  else mk_abs (v,tm0') //if something goes wrong consider that I've changed tm' == tm and tm2' == tm2
           with Clash v_ ->
             (* Lambda abstraction with clash in subterm *)
             let () = assert0 (term_eq v v_) (Clash v_) (* reraise if not 'v' *)
             let foo v = try  free_vars (assoc v theta)  with HolFail _ -> [v]
             let avs = flatten (map foo (free_vars tm0))
             let v' = cvariant avs v in           (* generate fresh name for 'v' *)
             let tm0' = var_inst0 vs0 ((v,v')::theta') tm0
             mk_abs (v',tm0'))
...
let var_inst theta tm =
    ...
    | _  -> let () = assert1 (forall (is_var <* fst) theta)
                                (func, "Non-variable in instantiation domain")
            let () = assert1
                        (forall (fun (v,tm') ->(type_eq (type_of tm') (type_of v)))
                                theta)
                        (func, "Non-equal types in instantiation list")
            var_inst0 [] theta tm
    with Clash _ -> internal_err func
...
let rec tyvar_inst0 theta0 tytheta tm =
  match (dest_term tm) with
    Term_var (x,ty)
       -> let ty' = type_inst tytheta ty
          let tm' = if (ty' = ty)  then tm  else mk_var (x,ty') //if something goes wrong consider that I've changed ty' == ty
          (
           try
             let v0 = inv_assoc tm' theta0      (* HolFail if 'tm'' not bound *)
             if (term_eq v0 tm)
  ...
  | Term_comb (tm1,tm2)
       -> let tm1' = tyvar_inst0 theta0 tytheta tm1
          let tm2' = tyvar_inst0 theta0 tytheta tm2
          (* Function application with no clash in subterms *)
          if (tm1' = tm1) && (tm2' = tm2)  then tm  else mk_comb (tm1',tm2') //if something goes wrong consider that I've changed tm1' == tm1 e tm2' == tm2
  | Term_abs (v,tm0)
       -> let v' = tyvar_inst0 [] tytheta v
          (
           try
             let tm0' = tyvar_inst0 ((v,v')::theta0) tytheta tm0  (* See Note *)
             (* Lambda abstraction with no clash in subterm *)
             if (v' = v) && (tm0' = tm0)  then tm  else mk_abs (v',tm0') //if something goes wrong consider that I've changed v' == v e tm0' == tm0
           with Clash v_ ->
             (* Lambda abstraction with clash in subterm *)
             let () = assert0 (term_eq v v_)
                              (Clash v_)         (* reraise if not for 'v' *)
             let v' = cvariant (free_vars tm0) v (* generate fresh name for 'v'*)
             let tm0' = var_inst0 [] [(v,v')] tm0
             let v'' = tyvar_inst0 [] tytheta v'
             let tm0'' = tyvar_inst0 theta0 tytheta tm0'
             mk_abs (v'',tm0'')
          )
...
let tyvar_inst tytheta tm =
    ...
    try
     ...
     | _  -> let () = assert1 (forall (is_var_type <* fst) tytheta)
                             (func,"Non-type-variable in instantiation domain")
