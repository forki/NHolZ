...
(* becomes the definition term that is passed for basic constant definition.  *)
(* Note that if there are no LHS arguments, then this is the same as the      *)
(* constant definition style defined in the 'Def1' module.                    *)


(* Function definition database *)

let the_fun_defs = ref (dltree_empty : dltree<string,thm>)

let get_fun_definition x =
    try
        dltree_lookup x !the_fun_defs
    with HolFail _ ->
        let func = "get_fun_definition"
...
let new_fun_definition tm =       (* !vs. f u1 u2 .. = t     *)
    ...
    try
        ...
        let th0s = map (sym_rule <* fst_snd_conv) tm0s in
                                                   (* |- vi= vi[theta1;theta2]     *)
        let th4 = subs_conv th0s rhs in            (* |- t = t[theta1;theta2]      *)
        let th5 = trans_rule th3 (sym_rule th4) in (* |- f u1 u2 .. = t            *)
