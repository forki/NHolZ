...
(* constant arity as an integer as opposed to a big integer.                  *)
...
(* It takes an integer for the type constant arity, instead of the big        *)
(* inteter taken by the primitive command.                                    *)
  ...
  let th1 =
     if (is_comb p) &&
       ...
       then th                         (* |- ?v. p             *)
            ...
            let th1 = mk_comb_rule (refl_conv ex)
                                   (mk_abs_rule v (beta_conv p0)) in
                                                (* |- ?v. (\v. p) v <=> ?v. p *)
            let th2 = refl_conv (mk_exists (v,p0)) in
            eq_mp_rule
              (eq_mp_rule (mk_comb_rule (mk_comb_rule (refl_conv e) th1) th2)
                          th2)
              th in                    (* |- ?v. (\v. p) v     *)
  try
