...
let mk_pair_rep_lemma =
  let c = (parse_term(@"c:'a")) 
  let d = (parse_term(@"d:'b")) in
  (* |- !c d. ?a b. MkPairRep c d = MkPairRep a b   *)
  list_gen_rule [c;d]
    (exists_rule ((parse_term(@"?(a:'a) (b:'b). MkPairRep c d = MkPairRep a b")), c)
      (exists_rule ((parse_term(@"?b. MkPairRep (c:'a) (d:'b) = MkPairRep c b")), d)
        (refl_conv (parse_term(@"MkPairRep (c:'a) (d:'b)"))) ))
...
let pair_rep_exists_lemma =
  let c = (parse_term(@"c:'a")) 
  let d = (parse_term(@"d:'b")) 
  let r = (parse_term(@"r : 'a->'b->bool")) in
  (* |- ?r. IsPairRep r                                    *)
  eq_mp_rule
    ...
    (exists_rule
              ((parse_term(@"?r (a:'a) (b:'b). r = MkPairRep a b")), (parse_term(@"MkPairRep (c:'a) (d:'b)")))
...
let mk_pair_rule th1 th2 =
    try
        ...
        let ty2 = type_of (eqthm_rhs th2)  in
        let f = mk_iconst ("PAIR",[(a_ty,ty1);(b_ty,ty2)]) in
...
let rep_abs_pair_lemma =
  ...
  let y = (parse_term(@"y:'b")) in
  let r = (parse_term(@"MkPairRep (x:'a) (y:'b)")) in
  (* |- !x y. PairRep (PairAbs (MkPairRep x y)) = MkPairRep x y  *)
  list_gen_rule [x;y]
    (eq_mp_rule
      (trans_rule
        ...
        (sym_rule (app_beta_rhs_rule is_pair_rep_def r))
        (* |- IsPairRep (MkPairRep x y) <=>                       *)
...
let pair_eq_thm = 
    save_thm ("pair_eq_thm",
      let x = (parse_term(@"x:'a")) 
      let y = (parse_term(@"y:'b"))
      let u = (parse_term(@"u:'a")) 
      let v = (parse_term(@"v:'b"))
      let repfn = mk_gconst "PairRep" in
      let th1 = assume_rule (parse_term(@"(x:'a)=u /\ (y:'b)=v")) in
                                         (* x = u /\ y = v |- x = u /\ y = v    *)
          ...
          (eq_mp_rule
            ...
            (list_app_beta_rule
              ...
              (list_trans_rule [
                ...
                sym_rule (list_app_beta_rhs_rule mk_pair_rep_def [x;y]);
                (* |- ...                = PairRep (PairAbs (MkPairRep x y))  *)
                sym_rule (list_spec_rule [x;y] rep_abs_pair_lemma);
                ...
                mk_comb2_rule repfn
                  (eq_mp_rule
                    ...
                    (assume_rule (parse_term(@"((x:'a),(y:'b))=(u,v)"))) );
                (* |- ...                = MkPairRep u v                      *)
...
let pair_surjective_thm = 
    save_thm ("pair_surjective_thm",
      ...
      let b = (parse_term(@"b:'b"))
      let p = (parse_term(@"p:'a#'b"))
      let absfn = mk_gconst "PairAbs" 
      let repfn = mk_gconst "PairRep" in
        ...
        (prove_asm_rule
          ...
          (eq_mp_rule
            (trans_rule
              ...
              (*      <=> IsPairRep (PairRep p)                       *)
              (sym_rule
                (spec_rule (parse_term(@"PairRep (p:'a#'b)")) prod_bij_def2) )
              (* |- IsPairRep (PairRep p) <=> (?a b. PairRep p = MkPairRep a b)  *)
              (app_beta_rhs_rule is_pair_rep_def (parse_term(@"PairRep (p:'a#'b)"))) )
            (* |- PairRep (PairAbs (PairRep p)) = PairRep p    *)
            (mk_comb2_rule repfn th1) )
          (* ?a b. PairRep p = MkPairRep a b |- ?x y. p = (x,y)   *)
          (choose_rule (a, assume_rule (parse_term(@"?a b. PairRep (p:'a#'b) = MkPairRep a b")))
            (choose_rule (b, assume_rule (parse_term(@"?b. PairRep (p:'a#'b) = MkPairRep a b")))
              (* PairRep p = MkPairRep a b |- ?x y. p = (x,y)         *)
              (exists_rule ((parse_term(@"?x y. (p:'a#'b) = (x,y)")), a)
                (exists_rule ((parse_term(@"?y. (p:'a#'b) = (a,y)")), b)
                  ...
                  (list_trans_rule [
                    ...
                    mk_comb2_rule absfn
                      (assume_rule (parse_term(@"PairRep (p:'a#'b) = MkPairRep a b")));
                    (* |- PairAbs (MkPairRep a b) = (a,b)                   *)
                    sym_rule (list_app_beta_rhs_rule pair_def [a;b])
                    ] ))))))
    )
...
let fst_thm = 
    save_thm ("fst_thm",
      ...
      list_gen_rule [x;y]
        (list_trans_rule [
          (* |- FST (x,y) = (@x'. ?y'. (x,y) = (x',y'))             *)
          app_beta_rhs_rule fst_def (parse_term(@"((x:'a),(y:'b))"));
          ...
          sym_rule
            ...
            (choose_rule (v,
              ...
              select_rule
                (* |- ?u v. x=u /\ y=v                                    *)
                (exists_rule ((parse_term(@"?(u:'a) (v:'b). x = u /\ y = v")), x)
                  (exists_rule ((parse_term(@"?(v:'b). x = (x:'a) /\ y = v")), y)
                    (* |- x = x /\ y = y                                       *)
                    (conj_rule (refl_conv x) (refl_conv y)) )) )
              (* x = (@u. ?v. x=u /\ y=v) /\ y = v |- x = (@u. ?v. x=u /\ y=v) *)
              (conjunct1_rule
                (assume_rule (parse_term(@"x = (@(u:'a). ?(v:'b). x=u /\ y=v) /\ y = v"))) ))
          ] )
    )
...
let snd_thm = 
    save_thm ("snd_thm",
      ...
      list_gen_rule [x;y]
       (list_trans_rule [
         (* |- SND (x,y) = (@y'. ?x'. (x,y) = (x',y'))        *)
         app_beta_rhs_rule snd_def (parse_term(@"((x:'a),(y:'b))"));
         ...
         sym_rule
           ...
           (choose_rule (u,
             ...
             (select_rule
               (* |- ?v u. x=u /\ y=v                               *)
               (exists_rule ((parse_term(@"?(v:'b) (u:'a). x = u /\ y = v")), y)
                 (exists_rule ((parse_term(@"?(u:'a). x = u /\ (y:'b) = y")), x)
                   (* |- x=x /\ y=y                                     *)
                   (conj_rule (refl_conv x) (refl_conv y)) ))) )
             (* x = u /\ y = (@v. ?u. x=u /\ y=v) |- y = (@v. ?u. x=u /\ y=v)  *)
             (conjunct2_rule
               (assume_rule (parse_term(@"x = u /\ y = (@(v:'b). ?(u:'a). x=u /\ y=v)"))) ))
         ] )
    )
    ...
    save_thm ("fst_snd_thm",
      ...
      gen_rule p
        (prove_asm_rule
          ...
          (choose_rule (x,
             assume_rule (parse_term(@"?x y. (p:'a#'b) = (x,y)")))
            (choose_rule (y,
               assume_rule (parse_term(@"?y. (p:'a#'b) = (x,y)")))
              (* p = (x,y) |- (FST p, SND p) = p               *)
              (subs_rule
                (* p = (x,y) |- (x,y) = p                        *)
                [sym_rule (assume_rule (parse_term(@"(p:'a#'b) = (x,y)")))]
                (* |- (FST (x,y), SND (x,y)) = (x,y)             *)
                (mk_pair_rule
