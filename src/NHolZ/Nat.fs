...
let ind_zero_is_nat_rep_lemma =
    let p = (parse_term(@"P:ind->bool")) in
    (* |- IsNatRep IND_ZERO           *)
    eq_mp_rule
      (* |- (!P. P IND_ZERO /\ (!j. P j ==> P (IND_SUC j)) ==> P IND_ZERO)   *)
      (*      <=> IsNatRep IND_ZERO                                          *)
      (sym_rule (spec_rule (parse_term(@"IND_ZERO")) is_nat_rep_def))
      (* |- !P. P IND_ZERO /\ (!j. P j ==> P (IND_SUC j)) ==> P IND_ZERO     *)
      (gen_rule p
        (disch_rule (parse_term(@"P IND_ZERO /\ (!j. P j ==> P (IND_SUC j))"))
          (conjunct1_rule
            (assume_rule (parse_term(@"P IND_ZERO /\ (!j. P j ==> P (IND_SUC j))"))) )))
...
let ind_suc_is_nat_rep_lemma =
    let p = (parse_term(@"P:ind->bool")) in
    let i = (parse_term(@"i:ind")) in
    (* |- !i. IsNatRep i ==> IsNatRep (IND_SUC i)           *)
    gen_rule i
      (disch_rule (parse_term(@"IsNatRep i"))
        (* IsNatRep i |- IsNatRep (IND_SUC i)                   *)
        (eq_mp_rule
          (sym_rule (spec_rule (parse_term(@"IND_SUC i")) is_nat_rep_def))
          (* IsNatRep i |-                                                      *)
                ...
                (undisch_rule
                  (spec_rule p
                    (eq_mp_rule
                      ...
                      (assume_rule (parse_term(@"IsNatRep i"))) ))))))))
...
let nat_rep_exists_lemma =
    (* |- ?x. IsNatRep x        *)
    exists_rule ((parse_term(@"?x. IsNatRep x")), (parse_term(@"IND_ZERO")))
      (* |- IsNatRep IND_ZERO     *)
      ind_zero_is_nat_rep_lemma
...
let is_nat_rep_lemma =
    let n = (parse_term(@"n:nat")) in
    (* |- !n. IsNatRep (NatRep n)            *)
    gen_rule n
      (eq_mp_rule
        (* |- NatRep (NatAbs (NatRep n)) = NatRep n <=> IsNatRep (NatRep n)  *)
        (sym_rule (spec_rule (parse_term(@"NatRep n")) nat_bij_def2))
        (mk_comb2_rule (parse_term(@"NatRep"))
          (spec_rule (parse_term(@"n:nat")) nat_bij_def1) ))
...
let nat_rep_injective_lemma =
    ...
    list_gen_rule [m;n]
      (deduct_antisym_rule
        ...
        (mk_comb2_rule (parse_term(@"NatRep")) (assume_rule (parse_term(@"(m:nat) = n"))))
        ...
        (eq_mp_rule
          ...
          (mk_comb2_rule (parse_term(@"NatAbs")) (assume_rule (parse_term(@"NatRep m = NatRep n")))) ))
...
let nat_rep_zero_lemma =
    ...
    trans_rule
      ...
      (mk_comb2_rule (parse_term(@"NatRep")) zero_def)
      (* |- NatRep (NatAbs IND_ZERO) = IND_ZERO      *)
      (eq_mp_rule
        (spec_rule (parse_term(@"IND_ZERO")) nat_bij_def2)
        ind_zero_is_nat_rep_lemma )
...
let nat_rep_suc_lemma =
    ...
    gen_rule n
      (trans_rule
        ...
        (eq_mp_rule
          (spec_rule (parse_term(@"IND_SUC (NatRep n)")) nat_bij_def2)
          (* |- IsNatRep (IND_SUC (NatRep n))                              *)
          (mp_rule
            (spec_rule (parse_term(@"NatRep n")) ind_suc_is_nat_rep_lemma)
...
let suc_not_zero_thm0 =
  ...
  gen_rule n
    (not_intro_rule
      (disch_rule (parse_term(@"SUC n = ZERO"))
        (* SUC n = ZERO |- false                           *)
        (eq_mp_rule
          ...
          (trans_rule
            ...
            (eqf_intro_rule
              (spec_rule (parse_term(@"NatRep n")) ind_suc_not_zero_thm) ))
          (* SUC n = ZERO |- NatRep (SUC n) = NatRep ZERO    *)
          (mk_comb2_rule (parse_term(@"NatRep")) (assume_rule (parse_term(@"SUC n = ZERO"))) ))))

(* suc_injective_thm : thm                                                    *)
(*                                                                            *)
(*    |- !m n. SUC m = SUC n <=> m = n                                        *)

let suc_injective_thm = 
    save_thm ("suc_injective_thm",
      let m = (parse_term(@"m:nat")) 
      let n = (parse_term(@"n:nat")) in
      (* !m n. SUC m = SUC n <=> m = n                           *)
      list_gen_rule [m;n]
        (list_trans_rule [
          ...
          sym_rule (list_spec_rule [(parse_term(@"SUC m"));(parse_term(@"SUC n"))] nat_rep_injective_lemma);
          (* |- ...           <=> IND_SUC (NatRep m) = IND_SUC (NatRep n)   *)
          mk_eq_rule
            (spec_rule m nat_rep_suc_lemma)
            (spec_rule n nat_rep_suc_lemma);
          (* |- ...           <=> NatRep m = NatRep n                       *)
          list_spec_rule [(parse_term(@"NatRep m"));(parse_term(@"NatRep n"))] ind_suc_injective_thm;
          (* |- ...           <=> m = n                                     *)
...
let nat_induction_thm0 =
  let j = (parse_term(@"j:ind")) 
  let n = (parse_term(@"n:nat")) 
  let p = (parse_term(@"P:nat->bool")) in
  let th1 = assume_rule (parse_term(@"P ZERO /\ (!n. P n ==> P (SUC n))")) in
  let th2 = assume_rule (parse_term(@"IsNatRep j /\ P (NatAbs j)")) in
  (* |- !P. P ZERO /\ (!n. P n ==> P (SUC n)) ==> (!n. P n)    *)
  gen_rule p
    (disch_rule (parse_term(@"P ZERO /\ (!n. P n ==> P (SUC n))"))
      (gen_rule n
        ...
        (eq_mp_rule
          ...
          (conjunct2_rule
            ...
            (mp_rule
              (bspec_rule (parse_term(@"\i. IsNatRep i /\ (P:nat->bool) (NatAbs i)"))
                (* |- !P. P IND_ZERO /\ (!j. P j ==> P (IND_SUC j))           *)
                (*        ==> P (NatRep n))                                   *)
                (eq_mp_rule
                  (spec_rule (parse_term(@"NatRep n")) is_nat_rep_def)
                  (spec_rule n is_nat_rep_lemma) ))
              (conj_rule
                (* P ZERO /\ (!n. P n ==> P (SUC n))                      *)
                (*    |- IsNatRep IND_ZERO /\ P (NatAbs IND_ZERO)         *)
                (eq_mp_rule
                  (mk_conj2_rule (parse_term(@"IsNatRep IND_ZERO"))
                ...
                (gen_rule j
                  (disch_rule (parse_term(@"IsNatRep j /\ P (NatAbs j)"))
                    (conj_rule
                      ...
                      (eq_mp_rule
                        ...
                        (mk_comb2_rule p
                          (trans_rule
                            ...
                            (spec_rule (parse_term(@"NatAbs j")) suc_def)
                            (mk_comb2_rule (parse_term(@"NatAbs"))
                        ...
                        (mp_rule
                          ...
                          (spec_rule (parse_term(@"NatAbs j")) (conjunct2_rule th1))
                          (conjunct2_rule th2) )))))))))))
...
let nat_cases_thm0 =
  ...
  mp_rule
    ...
    (conj_rule
      ...
      (disj1_rule (refl_conv (parse_term(@"ZERO"))) (parse_term(@"?n. ZERO = SUC n")))
      (* |- !n. n = ZERO \/ (?m. n = SUC m)                       *)
      (*        ==> SUC n = ZERO \/ (?m. SUC n = SUC m)           *)
      (gen_rule n
        (disch_rule (parse_term(@"n = ZERO \/ (?m. n = SUC m)"))
          (* |- SUC n = ZERO \/ (?m. SUC n = SUC m)                   *)
          (disj2_rule (parse_term(@"SUC n = ZERO"))
            (exists_rule ((parse_term(@"?m. SUC n = SUC m")), n)
              (refl_conv (parse_term(@"SUC n"))) )))))
...
let e = (parse_term(@"e:'a")) 
let f = (parse_term(@"f:'a->nat->'a"))
let fn_ = (parse_term(@"fn:nat->'a"))
let prg = (parse_term(@"PRG:nat->'a->bool")) 
let prg' = (parse_term(@"PRG':nat->'a->bool"))
let m = (parse_term(@"m:nat"))
let n = (parse_term(@"n:nat")) 
let n' = (parse_term(@"n':nat"))
...
let y1 = (parse_term(@"y1:'a")) 
let y2 = (parse_term(@"y2:'a"))

let f_x_n = (parse_term(@"(f:'a->nat->'a) x n"))
let f_y_n = (parse_term(@"(f:'a->nat->'a) y n"))
let prg_n_x = (parse_term(@"(PRG:nat->'a->bool) n x"))
let fn_n = (parse_term(@"(fn:nat->'a) n"))

(* Intermediate assumptions *)

let prg_case1_tm = (parse_term(@"m = ZERO /\ (y:'a) = e"))
let prg_case2_tm_body_tm = (parse_term(@"m = SUC n /\ y = (f:'a->nat->'a) x n /\ PRG n x"))
let prg_case2_tm_body1_tm = (parse_term(@"?n. m = SUC n /\ (y:'a) = f (x:'a) n /\ PRG n x"))
let prg_case2_tm = (parse_term(@"?(x:'a) n. m = SUC n /\ (y:'a) = f x n /\ PRG n x"))

let prg_casesdef_body =
    (parse_term(@"m = ZERO /\ (y:'a) = e \/  (?(x:'a) n. m = SUC n /\ (y:'a) = f x n /\ PRG n x)"))

let prg_casesdef_tm =
    (parse_term(@"!(m:nat) (y:'a).
                  PRG m y <=>
                     m = ZERO /\ y = e \/
...
let prg_recdef_tm =
    (parse_term(@"PRG ZERO e /\ (!(x:'a) n. PRG n x ==> PRG (SUC n) (f x n))"))
...
let lemma1 =
  let tm1a = (parse_term(@"!(m:nat) (y:'a). PRG m y ==> PRG' m y")) in
  let tm1b = (parse_term(@"?(x:'a) n. m = SUC n /\ (y:'a) = f x n /\ PRG' n x")) in
       ...
       (disj2_rule prg_case1_tm
         (choose_rule (x, assume_rule prg_case2_tm)
           (choose_rule (n, assume_rule prg_case2_tm_body1_tm)
             (exists_rule (tm1b, x)
               (exists_rule
                      ((parse_term(@"?n. m = SUC n /\ (y:'a) = f (x:'a) n /\ PRG' n x")),n)
                 (conj_rule
...
let lemma2 =
  let tm2 = (parse_term(@"!PRG'. (!m (y:'a). m = ZERO /\ y = e \/
                               (?(x:'a) n. m = SUC n /\ y = f x n /\ PRG' n x)
                               ==> PRG' m y)
                    ==> PRG' m y")) in
  ...
  let tm2e = (parse_term(@"\(m:nat) (y:'a).
                 m = ZERO /\ y = e \/
                 (?(x:'a) n. m = SUC n /\ y = f x n /\ PRG n x)")) in
  let tm2f = (parse_term(@"(?(x:'a) n. m = SUC n /\ (y:'a) = f x n /\ PRG n x) ==> PRG m y"))in
  ...
  let th2c = (* <PRG-fpdef>,                                        *)
             ...
             (* |- !m y. PRG m y ==> PRG' m y                       *)
             list_gen_rule [m;y]
              (disch_rule (parse_term(@"(PRG:nat->'a->bool) m y"))
                (undisch_rule
   ...
   (exists_rule (exists_prg_defconj_tm,prg)
     (conj_rule
       ...
       (prove_asm_rule
         ...
         (*    (!x n. PRG n x ==> PRG (SUC n) (f x n))        *)
         (conj_rule
           (mp_rule (inst_rule [(m,(parse_term(@"ZERO")));(y,e)]
                  (disch_rule prg_case1_tm (undisch_rule
                    (prove_asm_rule
                      (disj1_rule (assume_rule prg_case1_tm) prg_case2_tm)
                      th2a ) )))
               (conj_rule (refl_conv (parse_term(@"ZERO"))) (refl_conv e)) )
           (list_gen_rule [x;n] (disch_rule prg_n_x
             (mp_rule (inst_rule [(m,(parse_term(@"SUC n")));(y,f_x_n)]
                   (prove_asm_rule
                       ...
                       (mp_rule (assume_rule tm2f)
                           (exists_rule (prg_case2_tm,x)
                             (exists_rule (prg_case2_tm_body1_tm,n)
                               (assume_rule prg_case2_tm_body_tm))) ) )))
                 (conj_rule (refl_conv (parse_term(@"SUC n")))
                   (conj_rule (refl_conv f_x_n) (assume_rule prg_n_x))) ))) ))
...
let lemma3 =
  ...
  let tm3a = (parse_term(@"?!(x:'a). PRG (n:nat) x")) in
  let tm3b = (parse_term(@"y = (f:'a->nat->'a) x n /\ PRG n x")) in
  let tm3c = (parse_term(@"(?(x:'a). (y1:'a) = f x (n:nat) /\ PRG n x) /\
              (?x. y2 = f x n /\ PRG n x)")) in
  let tm3d = (parse_term(@"(y:'a) = e")) in
  let tm3e = (parse_term(@"?(x:'a) (n':nat).
                 n' = n /\ y = (f:'a->nat->'a) x n' /\ PRG n' x")) in
  let th3a = assume_rule (parse_term(@"y1 = (f:'a->nat->'a) x n /\ PRG n x")) in
  let th3b = assume_rule (parse_term(@"y2 = (f:'a->nat->'a) x' n /\ PRG n x'")) in
  let th3c = assume_rule tm3c in
  let th3d = spec_rule (parse_term(@"false")) exists_null_thm in(* |- (?x. false) <=> false  *)
  let th3e = eqf_intro_rule                      (* |- SUC n = ZERO <=> false *)
               (spec_rule n suc_not_zero_thm0) in
  let th3f = eq_mp_rule
               (bspec_rule (parse_term(@"\x. (PRG:nat->'a->bool) n x")) uexists_thm3)
               (assume_rule tm3a) in
  let th3g = assume_rule prg_casesdef_tm in

  mp_rule

   (bspec_rule (parse_term(@"\(n:nat). ?!(y:'a). PRG n y"))
     (* |- !P. P ZERO /\ (!n. P n ==> P (SUC n)) ==> (!n. P n)    *)
     nat_induction_thm0 )
     ...
     (eq_mp_rule
       (sym_rule
         (mk_uexists_rule y
           ...
           (* |- PRG ZERO y <=> y = e                       *)
           (list_trans_rule [
              list_spec_rule [(parse_term(@"ZERO"));y] th3g;
              mk_disj_rule
               (list_trans_rule
                  [ mk_conj1_rule (eqt_intro_rule (refl_conv (parse_term(@"ZERO")))) tm3d;
                    list_spec_rule [(parse_term(@"true"));tm3d] conj_comm_thm;
                    spec_rule tm3d conj_id_thm ] )
               (trans_rule (mk_exists_rule x
                 (trans_rule (mk_exists_rule n
                   (list_trans_rule
                      [ mk_conj1_rule
                         (trans_rule (sym_conv (parse_term(@"ZERO = SUC n"))) th3e)
                         tm3b;
                        list_spec_rule [(parse_term(@"false"));tm3b] conj_comm_thm;
       ...
       (eqt_elim_rule
         (trans_rule
           (sym_rule (mk_uexists_rule x (spec_rule (parse_term(@"(x:'a)=e")) conj_id_thm)))
           (list_bspec_rule [(parse_term(@"\(x:'a).true"));e] uexists_one_point_thm) )))
         ...
         (eq_mp_rule
           (sym_rule
             (mk_uexists_rule y
               ...
               (list_trans_rule
                  [ list_spec_rule [(parse_term(@"SUC n"));y] th3g;
                    mk_disj_rule
                     (list_trans_rule
                        [ mk_conj1_rule th3e tm3d;
                          list_spec_rule [(parse_term(@"false"));tm3d] conj_comm_thm;
                          spec_rule tm3d conj_zero_thm ])
                     (mk_exists_rule x
                       (mk_exists_rule n'
                         (mk_conj1_rule
                           (trans_rule (list_spec_rule [n;n'] suc_injective_thm)
                                  (sym_conv (parse_term(@"(n:nat) = n'"))) )
                           (parse_term(@"y = (f:'a->nat->'a) x n' /\ PRG n' x")) )));
                    list_spec_rule [(parse_term(@"false"));tm3e] disj_comm_thm;
                    spec_rule tm3e disj_id_thm;
                    mk_exists_rule x
                     (list_bspec_rule
                        [(parse_term(@"\(n':nat). (y:'a) = f (x:'a) n' /\ PRG n' x"));n]
                        (inst_type_rule [a_ty,nat_ty] exists_one_point_thm) ) ])
               ))
           (* ?!x. PRG n x |- ?!y. ?x. y = f x n /\ PRG n x       *)
           (eq_mp_rule
             (sym_rule
               (bspec_rule (parse_term(@"\(y:'a). ?(x:'a). y = f x (n:nat) /\ PRG n x"))
                           uexists_thm3))
             (conj_rule
               (choose_rule (y, conjunct1_rule th3f)
                 (exists_rule
                         ((parse_term(@"?(y:'a) (x:'a). y = f x (n:nat) /\ PRG n x")),f_y_n)
                   (exists_rule
                         ((parse_term(@"?(x:'a). (f:'a->nat->'a) y n = f x n /\ PRG n x")),y)
                     (conj_rule (refl_conv f_y_n)
                                (assume_rule (parse_term(@"(PRG:nat->'a->bool) n y")))) )))
               (list_gen_rule [y1;y2] (disch_rule tm3c
               (choose_rule (x, conjunct1_rule th3c)
                 (choose_rule (x', conjunct2_rule th3c)
                   (eq_mp_rule
                     (sym_rule (mk_eq_rule (conjunct1_rule th3a)
                                           (conjunct1_rule th3b) ))
...
let lemma4 =
  let th4a = assume_rule (parse_term(@"!(n:nat) (y:'a). PRG n y <=> fn n = y")) in
  ...
  choose_rule (fn_,
   ...
   (exists_rule
         ((parse_term(@"?fn. fn ZERO = e /\
                (!n. fn (SUC n) = (f:'a->nat->'a) (fn n) n)")), fn_)
     (conj_rule
       (eq_mp_rule (list_spec_rule [(parse_term(@"ZERO"));e] th4a)
                   (conjunct1_rule th4b) )
       (gen_rule n
         (eq_mp_rule
           (list_spec_rule [(parse_term(@"SUC n"));(parse_term(@"(f:'a->nat->'a) (fn n) n"))] th4a)
           (mp_rule (list_spec_rule [fn_n;n] (conjunct2_rule th4b))
              (eq_mp_rule (sym_rule (list_spec_rule [n;fn_n] th4a))
                 (refl_conv fn_n) )))) ))
