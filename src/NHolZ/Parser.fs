...
(* Like the lexer, the syntax parsers are implemented in terms of reader      *)
(* functions that take a source (in this case, a list of lexical tokens) and  *)
...
(* The parsers take into account the fixity status of identifiers, as well as *)
(* certain specially-supported constants (whose declarations in later modules *)
...
let parse_list =
    (read_list : int -> (token list -> 'a * token list) -> token list -> 'a list * token list)

let parse_start =
    (read_start : token list -> unit * token list)

let parse_end =
    (read_end : token list -> unit * token list)
...
let syntax_err x = hol_err ("SYNTAX ERROR", x)

(* Error connectives *)

(* The mechanism for detecting/raising syntax errors involves use of a        *)
(* dedicated connective - either '/!' or '//!'.  These take a syntax reader   *)
...
(* whether or not there is an actual syntax error).  If a string is returned, *)
(* it gets used as the diagnosis message for a syntax error.                  *)

let ( /|/! ) parse_fn err_fn =
    (!|||) parse_fn (syntax_err @| (fun src -> (err_fn src, src)))
...
let rec no_close_err0 n (rw1,rw2) src = 
    let util1 = (!|||) (parse_resword rw2 *>> no_close_err0 (n-1) (rw1,rw2)) parse_end
    let util2 = (!|||) (parse_resword rw1 *>> no_close_err0 (n+1) (rw1,rw2)) util1
    (
      if (n = 0)
...
let parse_listA item rw parse_fn1 =
    ...
    (fun (x,xs) -> x::xs) @|
       (((!>>>) (parse_itemA item [rw] (parse_fn1 [item]))  (parse_list 0 (parse_fn1 items)))
...
(* The D-class is for items that occur in series, separated by given reserved *)
(* word 'sep' and surrounded by given bracket reserved words 'br1' and 'br2'. *)
...
(* This is for parsing a variable-length series of items, with minimum length *)
(* 'n', each item separated by resword 'sep' and series delimited at start    *)
(* and end by 'br1' and 'br2' respectively.                                   *)

let parse_listD0 n item (br1,sep,br2) parse_fn =
    (fun xs -> if (length xs < n) then syntax_err ("Missing " + item ()) else xs) 
    @|
    (
        let util0 = (parse_list (decrement n) (parse_reswordD (br1,sep,br2) sep *>> parse_itemD item (br1,sep,br2) parse_fn) >>* parse_resword br2)
        let util1 = (!>>>) (parse_itemD item (br1,sep,br2) parse_fn) (util0)
        (* Empty list *)
        (!|||) ((fun _ -> []) @| parse_resword br2) ((fun (x,xs) -> x::xs) @| util1)
    )
           ...
           ((n = n1) &&
            (if (h = NonAssoc)
               then let x = name_fn f1 in
                    syntax_err ("Missing bracket for " +
...
let rec build_infix_expr mk_bin_fn ys =
    ...
    | (Infix_arg expr)::ys0
         -> (expr,ys0)
    | [] -> internal_err "build_infix_expr"


//string * ('a -> string) -> (token list -> 'b * token list) * (token list -> ('a * 'c * 'd) * token list) -> (token list -> (('a * 'c * 'd) * 'b) list * token list) requires comparison
...
let parse_infix_expr (form,name_fn) (parse_fn,parse_op_fn,mk_bin_fn) e0 src =
    ...
    let _ = try assert (is_empty ys1)
            with Assert_failure _ -> internal_err "parse_infix_expr" in
    (e,src')

(* ** TYPE SYNTAX ANALYSIS ** *)

(* The type syntax parser takes a list of lexical tokens and produces a       *)
(* pretype.  The type quotation syntax extends beyond the primitive type      *)
...
//token list -> pretype * token list 
let rec parse_pretype2 (src:token list) = 
    (
        (!|||)
            (* Type variable *)
            ((fun x -> Ptyvar x) @| parse_name_with is_tyvar_token)
            (
                (!|||)
                    (* 0-arity compound type *)
                    ((fun x -> Ptycomp (x,[])) @| parse_name_with is_nonfix_tyconst_token)
                    (
                        (!|||)
                            (
                                let delims = ("(", ",", ")")
                                ...
                                (
                                    fun pty ->
                                        (!|||)
                                            (* Bracketed subtype *)
                                            (
                                                (fun _ -> pty) @| parse_reswordD delims ")")
                                            (
                                                (fun (ptys,x) -> Ptycomp (x, pty::ptys)) 
                                                @| 
                                                (!>>>) 
                                                    (parse_reswordD delims "," *>> parse_listD0 1 (fun() ->"type parameter") delims parse_pretype0) 
                                                    (parse_itemB (fun () -> "type constant") 
                                                        (parse_nonfix_tyconst /|/! ((fun x ->"Type constant " + quote x + " is not nonfix") @!| parse_name_with is_infix_tyconst_token)))
                                            )
                                )
                            )
                            (
                                (fun x -> syntax_err ("Term variable " + quote x + " encountered in type")) @|
                                    (parse_name_with (function Ident_tok (_,Tmvar_mark,_) -> true | _ -> false))
                            )
                    )
            )
    ) src
...
and parse_pretype1 src = 
    (
        ...
        (fun pty -> (fun xs -> foldl (fun pty1 x -> Ptycomp (x,[pty1])) pty xs) @| parse_list 1 parse_nonfix_tyconst)
    ) src

//token list -> pretype * token list 
and parse_pretype0 src = 
    (
        (parse_pretype1    /|/! ((fun x -> "Missing LHS for infix type " + quote x) @!| parse_name_with is_infix_tyconst_token))
        |@| 
        (* Infix compound type *)
        parse_infix_expr ("infix type",id_fn) (parse_pretype1, parse_infix_tyconst, mk_bin_pretype)
    ) src

let parse_pretype src = 
    fst 
        (
            try 
                (
                    (parse_start /! "Empty type quotation") 
                    *>> (parse_pretype0  /|/! hit_resword_err leading_type_reswords (fun()->"at start of type")) 
                    >>* (parse_end   /|/! hit_resword_err [] (fun()->"after syntax-correct leading subtype")) 
                ) src
            with ReadFail -> syntax_err "Undiagnosed type syntax error"
        )

(* ** TERM SYNTAX ANALYSIS ** *)

(* The term syntax parser takes a list of lexical tokens and produces a       *)
(* preterm.  The term quotation syntax extends significantly beyond the       *)
(* primitive term categories to include support for:                          *)
...
let rec parse_atom_with test_fn src = 
    (
        (!|||)
            (* Variable *)
            (mk_null_var_preterm @| parse_name_with (fun tok -> (is_var_token tok) && (test_fn tok)))
            (
                (!|||)
                    (* Constant *)
                    (mk_null_const_preterm @| parse_name_with (fun tok -> (is_const_token tok) && (test_fn tok)))
                    (
                        (* Type-annotated atom *)
                            ...
                            (
                                (!>>>) 
                                     ...
                                     *>> (parse_atom_with test_fn 
                                                /|/! no_close_err ("(",")") 
                                                /|/! early_resword_err [":"] (fun () ->"type annotation subterm")) >>* parse_resword ":") 
                                    (parse_itemC (fun () ->"type annotation type") ("(",")") parse_pretype0 >>* parse_resword ")")
                            )
                    )
            )
    ) src
...
//token list -> preterm * token list 

let rec parse_preterm5 src = 
                    ...
                    (!|||)
                        (
                            ...
                            (fun x -> let x0 = (char_implode <* filter is_digit <* char_explode) x in
                                      mk_bigint_nat_preterm (big_int_of_string x0)) @|
                               parse_name_with is_numeral_token
                        )
                        (
                            (!|||)
                                                ...
                                                (!>>>)
                                                    (parse_resword "if"
                                                        *>> parse_itemA (fun () -> "conditional condition") ["then"] parse_preterm1
                                                        >>* parse_reswordA "then")
                                                    (parse_itemA (fun () -> "conditional then-branch") ["else"] parse_preterm1 >>* parse_reswordA "else")
                                            )
                                    ...
                                    (!|||)
                                        ...
                                        (
                                            let parse_let_binding_item =
                                                let item1 = (fun () -> "let-binding variable")
                                                (!>>>)
                                                    (
                                                    ...
                                                    (parse_itemA (fun()->"let-binding RHS") ["and";"in"] parse_preterm1)
                                            mk_let_preterm @|
                                            (
                                                (!>>>)
                                                    (parse_resword "let" *>> parse_listD0 1 (fun () -> "let-expression binding") ("let","and","in") parse_let_binding_item)
                                                        ...
                                                        (
                                                            (!|||)
                                                                ...
                                                                (
                                                                    ...
                                                                    (
                                                                        (!>>>)
                                                                            (parse_resword "\\" *>> parse_listA (fun()->"lambda binding variable") "." (parse_nonfix_var false)) 
                                                                    ...
                                                                    (!|||)
                                                                        ...
                                                                        (
                                                                            (!|||)
                                                                                ...
                                                                                ((fun _ -> raise LocalFail) @| lookahead (parse_atom_with is_infix_token))
                                                                                (
                                                                                    (!|||)
                                                                                        //delims
                                                                                        (
                                                                                        ...
                                                                                        (
                                                                                            (* Other error cases *)
                                                                                            (fun x -> syntax_err ("Type variable " + quote x +
...
let parse_preterm src = 
    fst 
        (
            ...
            with ReadFail -> syntax_err "Undiagnosed term syntax error"
        )
...
(* above parse functions for types/terms to be the functions that get applied *)
(* when the ML interpreter encounters type/term quotations respectively.      *)
...
let string_tail x i = FSharp.Compatibility.OCaml.String.sub x i (FSharp.Compatibility.OCaml.String.length x - i)

let rec skip_space x i =
    if (i < FSharp.Compatibility.OCaml.String.length x) && (is_whitespace_char (FSharp.Compatibility.OCaml.String.get x i))
    then skip_space x (i+1)
    else i

let expand_hol_quotation x =
    let i = skip_space x 0
    if (i < FSharp.Compatibility.OCaml.String.length x) && (FSharp.Compatibility.OCaml.String.get x i = ':')
    then 
        let i' = skip_space x (i+1)
