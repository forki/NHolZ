...
let sort_by_type_complexity ptms =
    let nptms = map (fun ptm -> (atom_type_complexity ptm, ptm)) ptms
    let nptms' = mergesort (fun (n1,ptm1) (n2,ptm2) -> (n1 < n2)) nptms
    map snd nptms'
...
(* all atoms returned retain their original detyped pretypes (because these   *)
(* ultimately get used to pinpoint individual atoms in the preterm).          *)
...
(*   cs     - All const atoms in the subterm                                  *)
...
let rec atom_info0 thetaC ptm =
    ...
    | Ptmconst (x,pty)
         -> ([],[],[],[ptm],pty,[])
    | Ptmcomb (ptm1,ptm2)
         -> let (ovs1,fvs1,bvs1,cs1,pty1,theta1) = atom_info0 thetaC ptm1 in
            let (ovs2,fvs2,bvs2,cs2,pty2,theta2) = atom_info0 thetaC ptm2 in
            let ovs = ovs1 @ ovs2 in
            let fvs = fvs1 @ fvs2 in
            let bvs = bvs1 @ bvs2 in
            let cs = union cs1 cs2 in
            let theta12 = theta1 @ theta2 in
            let (ptya,ptyb) = 
                try   
                    dest_fun_pretype pty1
            ...
            (ovs,fvs,bvs,cs,ptyb',theta)
    | Ptmabs (ptmv,ptm0)
         -> let v = preterm_var ptmv in
            let ptyv = (snd <* dest_var_preterm) v in
            let (ovs0,fvs0,bvs0,cs0,pty0,theta0) = atom_info0 thetaC ptm0 in
            let fvs00 = v::fvs0 in
            ...
            (ovs,fvs,bvs,cs0,pty',theta)
    | Ptmtyped (ptm0,pty)
         -> let (ovs0,fvs0,bvs0,cs0,pty0,theta0) = atom_info0 thetaC ptm0 in
            let theta = unify_pretypes0 theta0 (pty0,pty) in
            let pty' = pretype_inst theta pty0 in
            (ovs0,fvs0,bvs0,cs0,pty',theta)

let atom_info thetaC ptm =
    let (ovs0,fvs0,bvs0,cs0,_,theta0) = atom_info0 thetaC ptm in
...
(* var atoms ('bvs'), the const atoms ('cs') and an intermediate instn list   *)
(* for generated tyvars (deduced from the unannotated preterm and its         *)
(* overloaded var atoms).  These atoms all have their original "detyped"      *)
...
let pick_atom_coverage thetaC pty_ ptm =
    let ptm1 = match pty_ with Some pty -> Ptmtyped (ptm,pty)
                             | None     -> ptm in
    let (ovs,fvs,bvs,cs,theta) = atom_info thetaC ptm1 in
