...
let the_type_bijections =
    ref (dltree_empty :  dltree<string, (string * string) * (thm * thm)>)
...
let new_type_bijections x (tyfn1,tyfn2) =
    ...
    try
     ...
     let th5 = exists_rule (mk_exists (a',tm3), a) (refl_conv a) in
                                                    (* |- ?a'. a = a'           *)
     let th6 = select_rule th5 in                   (* |- a = (@a'. a = a')     *)
     let th7 = gen_rule a (list_trans_rule [th4; th3; sym_rule th6]) in
                             (* |- !a. (\r. @a. r = f a) (f a) = a              *)
     ...
     let th12 = trans_rule th11 (sym_rule th9) in
                             (* P r |- f ((\r. @a. r = f a) r) = r              *)
     let th13 = disch_rule tm2 th12 in
                             (* |- P r ==> f ((\r. @a. r = f a) r) = r          *)
     let tm7 = mk_exists (a,tm4) in    (* ?a. r = f a   *)
     let tm8 = mk_eq (mk_comb (f,tm6), r) in
     let th14 = exists_rule (tm7,tm6) (sym_rule (assume_rule tm8)) in
                             (* f ((\r. @a. r = f a) r) = r |- ?a. r = f a      *)
     let th15 = eq_mp_rule (sym_rule thy) th14 in
                             (* f ((\r. @a. r = f a) r) = r |- P r              *)
