...
let print_close_brkt () = ")"

let print_space () = " "

(* Conditional printer utils *)

let print_open_brkt_if b = if b then print_open_brkt () else ""

let print_close_brkt_if b = if b then print_close_brkt () else ""

let print_space_if b = if b then print_space () else ""
...
let print_sp_seplist sp print_fn print_sep_fn xs =
    let print_sep_fn1 () = (print_sep_fn () + print_space_if sp)
    ...
    | Ptycomp (x,ptys)
         -> if (has_infix_type_fixity x)
              then (* Infix compound type *)
                   ...
                   (print_seplist (print_pretype0 ([],false,pr1)) (print_sep ",") ptys) +
...
let rec print_full_preterm0 (fl0,pr0) ptm =
    if (is_nat_preterm ptm)
      then (* Numeral *)
           let n = dest_bigint_nat_preterm ptm in
           (string_of_big_int n)
      ...
      then (* Enumeration *)
           ...
           (br1) +
           (print_space ()) +
           (print_sp_seplist sp (print_full_preterm0 (false,pr1)) (print_sep ",") ptms) +
           (print_space ()) +
           (br2)
    else match ptm with
            ...
            | Ptmcomb _
                 -> if (is_cond_preterm ptm)
                      then (* Conditional expression *)
                           ...
                           (" then ") + 
                           (print_full_preterm0 (true,pr1) ptm1) +
                           (" else ") + 
                           (print_full_preterm0 (true,pr1) ptm2) +
                           (print_close_brkt_if (need_prec_brkts fl0 pr0 pr1))
                      ...
                      then (* Let-expression *)
                           ...
                           let pr1 = (10,0) in
                           let print_vtm (v,tm) =
                                  (print_full_preterm0 (true,pr1) v) + " = " +
                                  (print_full_preterm0 (true,pr1) tm)
                           (print_open_brkt_if (need_prec_brkts fl0 pr0 pr1)) +
                           ("let ") +
                           (print_seplist print_vtm (print_sep " and ") vtms) +
                           (" in ") + 
                           (print_full_preterm0 (true,pr1) tm0) +
                           (print_close_brkt_if (need_prec_brkts fl0 pr0 pr1))
                      ...
                      then (* Pair expression *)
                           ...
                           (print_open_brkt ()) +
                           (print_sp_seplist sp
                                   (print_full_preterm0 (false,pr1)) (print_sep ",") ptms) +
                           (print_close_brkt ())
                      ...
                      then (* Binder expression *)
                           ...
                           (print_open_brkt_if (need_prec_brkts fl0 pr0 pr1)) +
                           (print_atom f) + 
                           (print_space_if (need_separation [sf] s1)) +
                           (print_splist (print_full_preterm0 (false,pr1)) vs) +
                           (print_space_if (need_separation [2] s2)) + 
                           (". ") +
                           (print_full_preterm0 (false,pr1) ptm0) +
                           (print_close_brkt_if (need_prec_brkts fl0 pr0 pr1))
                           ...
                           let pr1 = (50,0) in
                           (print_open_brkt_if (need_prec_brkts fl0 pr0 pr1)) +
                           (print_full_preterm0 (false,pr1) f) + 
                           (print_space ()) +
                           (print_full_preterm0 (true,pr1) ptm0) +
                           (print_close_brkt_if (need_prec_brkts fl0 pr0 pr1))
            | Ptmabs _
                 -> (* Lambda abstraction *)
                    ...
                    (print_open_brkt_if (need_prec_brkts fl0 pr0 pr1)) +
                    ("\\") + 
                    (print_space_if (need_separation [2] s1)) +
                    (print_splist (print_full_preterm0 (false,pr1)) vs) +
                    (print_space_if (need_separation [2] s2)) + 
                    (". ") +
                    (print_full_preterm0 (false,pr1) ptm0) +
...
(* (using "$") if they have fixity, and enumerations and special case         *)
(* constants are printed without syntactic sugar.  Note that any type         *)
(* annotations occurring in the term are printed in full HOL type syntax.     *)
...
let rec print_prim_preterm0 (fl0,pr0) ptm =
    ...
    | Ptmabs _
         -> (* Lambda abstraction *)
            ...
            (print_space_if (need_separation [2] s2)) + "." +
            (print_space ()) + 
            (print_prim_preterm0 (false,pr1) ptm0) +
            (print_close_brkt_if (need_prec_brkts fl0 pr0 pr1))
    | Ptmtyped (ptm0,pty)
         -> (* Type-annotated expression *)
            let pr1 = (5,0) in
...
let print_type ty = (print_pretype <* type_to_pretype) ty
...
let print_term tm =
    (print_preterm <* printing_preterm false) tm
...
let print_thm th =
    ...
    (if (is_nonempty hs) then print_space () else "") +
    ("|-") + 
    (print_space ()) +
    (print_preterm c)
