...
let rec mk_bigint_nat0 n =
  if (gt_big_int n (big_int_of_string "0"))
    then let (n0,n1) = quomod_big_int n (big_int_of_string "2") in
         let f = if (eq_big_int n1 (big_int_of_string "0")) then bit0_fn
                                              else bit1_fn in
         let tm0 = mk_bigint_nat0 n0 in
         mk_comb (f,tm0)
  else if (eq_big_int n (big_int_of_string "0"))
    then zero_tm0
...
let rec dest_bigint_nat0 tm =
    try
      ...
      match x with
        "BIT0" -> mult_big_int (big_int_of_string "2") (dest_bigint_nat0 tm0)
      | "BIT1" -> add_big_int (mult_big_int (big_int_of_string "2") (dest_bigint_nat0 tm0))
                              (big_int_of_string "1")
      | _      -> hol_fail ("dest_bigint_nat","Not a numeral")
    with LocalFail ->
      if (term_eq tm zero_tm0)
        then (big_int_of_string "0")
