    ...
    with HolFail _ ->
        ...
        | Term_comb (tm1,tm2)
             -> let tm1' = subst0 vs0 theta tm1 in
                ...
                if (tm1' = tm1) && (tm2' = tm2)  then tm  else mk_comb (tm1',tm2') //if (tm1' == tm1) && (tm2' == tm2)  then tm  else mk_comb (tm1',tm2')
        | Term_abs (v,tm0)
             -> let theta' = fst_filter (not <* (var_free_in v)) theta in
                (try
                   let tm0' = subst0 (v::vs0) theta' tm0 in
                   if (tm0' = tm0)  then tm  else mk_abs (v,tm0') //if (tm0' == tm0)  then tm  else mk_abs (v,tm0')
                 with Clash v_ ->
