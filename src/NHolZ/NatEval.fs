...
let bit0_adddef_thm =
    ...
    mp_rule
      ...
      (conj_rule
        (* |- BIT0 0 = 0 + 0                                       *)
        (trans_rule (numeralise_mono_rule (conjunct1_rule bit0_def))
                    (sym_rule (spec_rule (parse_term(@"0")) add_id_thm)) )
        (* |- !n. BIT0 n = n + n ==> BIT0 (SUC n) = SUC n + SUC n  *)
        (gen_rule n
          (disch_rule (parse_term(@"BIT0 n = n + n"))
            (list_trans_rule
               [ (* |- BIT0 (SUC n) = SUC (SUC (BIT0 n))               *)
                 ...
                 mk_comb2_rule (parse_term(@"SUC"))
                  (mk_comb2_rule (parse_term(@"SUC"))
                    (assume_rule (parse_term(@"BIT0 n = n + n"))) );
                 (* |- SUC (SUC (n + n)) = SUC n + SUC n               *)
                 sym_rule
                  (trans_rule
                    (list_spec_rule [n;(parse_term(@"SUC n"))] add_dist_left_suc_thm)
                    (mk_comb2_rule (parse_term(@"SUC"))
...
let bit1_adddef_thm =
  gen_rule n
   (trans_rule
     ...
     (* |- SUC (BIT0 n) = SUC (n + n)      *)
     (mk_comb2_rule (parse_term(@"SUC")) (spec_rule n bit0_adddef_thm)) )
...
let bit0_multdef_thm =
  gen_rule n
    (trans_rule
      ...
      (* |- n + n = 2 * n                   *)
      (sym_rule (spec_rule n twice_thm)) )
...
let bit1_multdef_thm =
  gen_rule n
    (trans_rule
      ...
      (* |- ...    = SUC (2 * n)        *)
      (sym_rule (mk_comb2_rule (parse_term(@"SUC")) (spec_rule n twice_thm))) )
...
let bit1_multadddef_thm =
  gen_rule n
    (trans_rule
      ...
      (* |- ...    = 2 * n + 1          *)
      (spec_rule (parse_term(@"2*n")) suc_add_thm) )
...
let suc_bit0_lemma =        (* |- SUC (BIT0 n) = BIT1 n           *)
  trans_rule
    ...
    (mk_comb2_rule (parse_term(@"SUC")) (spec_rule n bit0_adddef_thm))
    (* |- SUC (n + n) = BIT1 n                   *)
    (sym_rule (spec_rule n bit1_adddef_thm))

let suc_bit1_lemma =        (* |- SUC (BIT1 n) = BIT0 (SUC n)     *)
  list_trans_rule
    [ (* |- SUC (BIT1 n) = SUC (SUC (n + n))     *)
      mk_comb2_rule (parse_term(@"SUC")) (spec_rule n bit1_adddef_thm);
      (* |- ...          = SUC n + SUC n         *)
      sym_rule
        (trans_rule
          (list_spec_rule [(parse_term(@"SUC n"));n] add_dist_right_suc_thm)
          (mk_comb2_rule (parse_term(@"SUC")) (list_spec_rule [n;n] add_dist_left_suc_thm)));
      (* |- ...          = BIT0 (SUC n)          *)
      sym_rule (spec_rule (parse_term(@"SUC n")) bit0_adddef_thm) ]
...
let rec eval_suc_conv0 tm0 =
    ...
    | Bit1 tm00
         -> (*  SUC (BIT1 n)  -->  BIT0 (SUC n)         *)
            trans_rule (inst_rule [(n,tm00)] suc_bit1_lemma)
...
let eval_pre_conv0 tm0 =
  let i = dest_bigint_nat0 tm0 in
  if (eq_big_int i (big_int_of_string "0"))
    then (*  PRE 0  -->  0                                    *)
         pre_zero_thm0
    else (*  [ SUC z = n ]  PRE n  -->  z                     *)
         let tm1 = mk_bigint_nat0 (pred_big_int i) in
         mp_rule
...
let zero_add_lemma =        (* |- ZERO + n = n                           *)
    trans_rule
      (list_spec_rule [(parse_term(@"ZERO"));n] add_comm_thm)
...
let bit0_add_bit0_lemma =   (* |- BIT0 m + BIT0 n = BIT0 (m + n)         *)
    list_trans_rule
      [ (* |- BIT0 m + BIT0 n = (m + m) + (n + n)      *)
        mk_bin_rule (parse_term(@"$+")) (spec_rule m bit0_adddef_thm)
                         (spec_rule n bit0_adddef_thm);
        (* |- ...             = (m + n) + (m + n)      *)
        mmnn_add_lemma;
        (* |- ...             = BIT0 (m + n)           *)
        sym_rule (spec_rule (parse_term(@"m+n")) bit0_adddef_thm) ]
    ...
    list_trans_rule
      [ (* |- BIT0 m + BIT1 n = (m + m) + SUC (n + n)   *)
        mk_bin_rule (parse_term(@"$+")) (spec_rule m bit0_adddef_thm)
                         (spec_rule n bit1_adddef_thm);
        (* |- ...             = SUC ((m+m) + (n+n))     *)
        list_spec_rule [(parse_term(@"m+m"));(parse_term(@"n+n"))] add_dist_right_suc_thm;
        (* |- ...             = SUC ((m+n) + (m+n))     *)
        mk_comb2_rule (parse_term(@"SUC")) mmnn_add_lemma;
        (* |- ...             = BIT1 (m+n)              *)
        sym_rule (spec_rule (parse_term(@"m+n")) bit1_adddef_thm) ]

let bit1_add_bit0_lemma =   (* |- BIT1 m + BIT0 n = BIT1 (m + n)         *)
    list_trans_rule
      [ (* |- BIT1 m + BIT0 n = SUC (m + m) + (n + n)   *)
        mk_bin_rule (parse_term(@"$+")) (spec_rule m bit1_adddef_thm)
                         (spec_rule n bit0_adddef_thm);
        (* |- ...             = SUC ((m+m) + (n+n))     *)
        list_spec_rule [(parse_term(@"m+m"));(parse_term(@"n+n"))] add_dist_left_suc_thm;
        (* |- ...             = SUC ((m+n) + (m+n))     *)
        mk_comb2_rule (parse_term(@"SUC")) mmnn_add_lemma;
        (* |- ...             = BIT1 (m+n)              *)
        sym_rule (spec_rule (parse_term(@"m+n")) bit1_adddef_thm) ]
    ...
    list_trans_rule
      [ (* |- BIT1 m + BIT1 n = SUC (m + m) + SUC (n + n)        *)
        mk_bin_rule (parse_term(@"$+")) (spec_rule m bit1_adddef_thm)
                         (spec_rule n bit1_adddef_thm);
        (* |- ...             = SUC (SUC ((m + m) + (n + n)))    *)
        list_spec_rule [(parse_term(@"m+m"));(parse_term(@"SUC (n+n)"))] add_dist_left_suc_thm;
        mk_comb2_rule (parse_term(@"SUC"))
          (list_spec_rule [(parse_term(@"m+m"));(parse_term(@"n+n"))] add_dist_right_suc_thm);
        (* |- ...             = SUC (SUC ((m + n) + (m + n)))    *)
        mk_comb2_rule (parse_term(@"SUC")) (mk_comb2_rule (parse_term(@"SUC")) mmnn_add_lemma);
        (* |- ...             = SUC (m + n) + SUC (m + n)        *)
        sym_rule
          (mk_comb2_rule (parse_term(@"SUC"))
                  (list_spec_rule [(parse_term(@"m+n"));(parse_term(@"m+n"))] add_dist_left_suc_thm));
        sym_rule
          (list_spec_rule [(parse_term(@"SUC (m+n)"));(parse_term(@"m+n"))] add_dist_right_suc_thm);
        (* |- ...             = BIT0 (SUC (m + n))               *)
       ...
       -> (*  BIT1 m + BIT1 n  -->  BIT1 (SUC (m + n))   *)
          ...
          trans_rule
            ...
            (mk_comb2_rule bit0_fn
              (trans_rule (mk_comb2_rule suc_fn th00)
...
let one_tm0 = (parse_term(@"BIT1 ZERO"))
...
let bit0_mult_lemma =      (* |- (BIT0 m) * n = BIT0 (m * n)   *)
    list_trans_rule
       [ (* |- BIT0 m * n = (2 * m) * n          *)
         mk_bin1_rule (parse_term(@"$*")) (spec_rule m bit0_multdef_thm) n;
         (* |- ...        = 2 * (m * n)          *)
         sym_rule (list_spec_rule [(parse_term(@"2"));m;n] mult_assoc_thm);
         (* |- ...        = BIT0 (m * n)         *)
         sym_rule (spec_rule (parse_term(@"m * n")) bit0_multdef_thm) ]

let mult_bit0_lemma =      (* |- m * (BIT0 n) = BIT0 (m * n)   *)
    list_trans_rule
       [ (* |- m * BIT0 n = m * (2 * n)          *)
         mk_bin2_rule (parse_term(@"$*")) m (spec_rule n bit0_multdef_thm);
         (* |- ...        = 2 * (m * n)          *)
         sym_rule (list_spec_rule [(parse_term(@"2"));m;n] (get_lemma "mult_left_flip_lemma"));
         (* |- ...        = BIT0 (m * n)         *)
         sym_rule (spec_rule (parse_term(@"m * n")) bit0_multdef_thm) ]
...
let bit1_mult_bit1_lemma =
  ...
  list_trans_rule
    [ (* |- (BIT1 m) * (BIT1 n) = (2*m + 1) * (2*n + 1)              *)
      mk_bin_rule (parse_term(@"$*")) (spec_rule m bit1_multadddef_thm)
                       (spec_rule n bit1_multadddef_thm);
      (* |- ...                 = (2*m) * (2*n + 1) + 1 * (2*n + 1)  *)
      list_spec_rule [(parse_term(@"2*m"));(parse_term(@"1"));(parse_term(@"2*n+1"))] mult_dist_left_add_thm;
      (* |- ...                 = (2 * (m * (2*n + 1)) + 2 * n) + 1  *)
      (mk_bin_rule (parse_term(@"$+"))
        (sym_rule (list_spec_rule [(parse_term(@"2"));m;(parse_term(@"2*n+1"))] mult_assoc_thm))
        (trans_rule (list_spec_rule [(parse_term(@"1"));(parse_term(@"2*n+1"))] mult_comm_thm)
                    (spec_rule (parse_term(@"2*n+1")) mult_id_thm) ));
      list_spec_rule [(parse_term(@"2*(m*(2*n+1))"));(parse_term(@"2*n"));(parse_term(@"1"))] add_assoc_thm;
      (* |- ...                 = 2 * (m * (2*n + 1) + n) + 1        *)
      sym_rule (mk_bin1_rule (parse_term(@"$+"))
          (list_spec_rule [(parse_term(@"2"));(parse_term(@"m*(2*n+1)"));n] mult_dist_right_add_thm) (parse_term(@"1")));
      (* |- ...                 = BIT1 (m * (2*n + 1) + n)           *)
      sym_rule (spec_rule (parse_term(@"m*(2*n+1) + n")) bit1_multadddef_thm);
      (* |- ...                 = BIT1 ((2*(m*n) + m) + n)           *)
      mk_comb2_rule (parse_term(@"BIT1"))
        (list_trans_rule
           [ (* |- m * (2 * n + 1) + n = (2 * (m * n) + m) + n        *)
             mk_bin1_rule (parse_term(@"$+"))
               (trans_rule
                 (list_spec_rule [m;(parse_term(@"2*n"));(parse_term(@"1"))] mult_dist_right_add_thm)
                 (mk_bin_rule (parse_term(@"$+"))
                   (list_spec_rule [m;(parse_term(@"2"));n] (get_lemma"mult_left_flip_lemma"))
                   (spec_rule m mult_id_thm) ) )
               n;
             (* |- ...                 = 2*(m*n) + (m + n)            *)
             sym_rule (list_spec_rule [(parse_term(@"2*(m*n)"));m;n] add_assoc_thm);
             (* |- ...                 = BIT0 (m*n) + (m + n)         *)
             mk_bin1_rule (parse_term(@"$+"))
               (sym_rule (spec_rule (parse_term(@"m*n")) bit0_multdef_thm)) (parse_term(@"m+n")) ])
     ]
      ...
      (mk_comb2_rule (parse_term(@"BIT1"))
        (list_trans_rule
           [ (* |- BIT0 (m * m) + (m + m) = 2 * (m*m) + 2 * m             *)
             mk_bin_rule (parse_term(@"$+"))
               (spec_rule (parse_term(@"m*m")) bit0_multdef_thm)
               (sym_rule (spec_rule m twice_thm));
             (* |- ...                    = 2 * (m * m + m)               *)
             sym_rule
               (list_spec_rule [(parse_term(@"2"));(parse_term(@"m*m"));m] mult_dist_right_add_thm);
             (* |- ...                    = BIT0 (m * m + m)              *)
...
let rec eval_mult_conv0 tm1 tm2 =
    ...
    | (Bit0 tm01, _)
         -> (*  BIT0 m * n  -->  BIT0 (m * n)                  *)
            trans_rule (inst_rule [(m,tm01);(n,tm2)] bit0_mult_lemma)
                       (mk_comb2_rule bit0_fn (eval_mult_conv0 tm01 tm2))
    | (_, Bit0 tm02)
         -> (*  m * BIT0 n  -->  BIT0 (m * n)                  *)
            trans_rule (inst_rule [(m,tm1);(n,tm02)] mult_bit0_lemma)
              ...
              then (*  BIT1 m * BIT1 m  -->  BIT1 (BIT0 (m*m + m))       *)
                   trans_rule
                     ...
                     (mk_comb2_rule bit1_fn
                       (mk_comb2_rule bit0_fn
                         (trans_rule (mk_bin1_rule (parse_term(@"$+")) th0 tm01)
                   ...
                   trans_rule
                     ...
                     (mk_comb2_rule bit1_fn
                       (trans_rule
                         (mk_bin_rule (parse_term(@"$+")) (mk_comb2_rule bit0_fn th0) th00)
...
let exp_bit0_lemma =     (* |- m EXP (BIT0 n) = (m EXP n) * (m EXP n)       *)
    trans_rule
      ...
      (mk_bin2_rule (parse_term(@"$EXP")) m (spec_rule n bit0_adddef_thm))
      (* |- ...            = (m EXP n) * (m EXP n)     *)
...
let exp_bit1_lemma =     (* |- m EXP (BIT1 n) = m * ((m EXP n) * (m EXP n)) *)
    list_trans_rule
       [ (* |- m EXP (BIT1 n) = m EXP (SUC (n + n))        *)
         mk_bin2_rule (parse_term(@"$EXP")) m (spec_rule n bit1_adddef_thm);
         (* |- ...            = m * (m EXP (n + n))        *)
         list_spec_rule [m;(parse_term(@"n+n"))] exp_dist_right_suc_thm;
         (* |- ...            = m * (m EXP n * m EXP n)    *)
         mk_bin2_rule (parse_term(@"$*")) m (list_spec_rule [m;n;n] exp_dist_right_add_thm) ]
         ...
         -> (*  m EXP (BIT0 n)  -->  (m EXP n) * (m EXP n)          *)
            ...
            list_trans_rule
               [ inst_rule [(m,tm1);(n,tm02)] exp_bit0_lemma;
                 mk_bin_rule (parse_term(@"$*")) th1 th1;
         ...
         -> (*  m EXP (BIT1 n)  -->   m * ((m EXP n) * (m EXP n))   *)
            ...
            list_trans_rule
               [ inst_rule [(m,tm1);(n,tm02)] exp_bit1_lemma;
                 mk_bin2_rule (parse_term(@"$*")) tm1
                   (trans_rule (mk_bin_rule (parse_term(@"$*")) th1 th1)
                               th2);
...
let bit0_even_lemma =           (* |- EVEN (BIT0 n) <=> true     *)
  trans_rule
    ...
    (mk_comb2_rule (parse_term(@"EVEN")) (spec_rule n bit0_multdef_thm))
    (* |- EVEN (2 * n) <=> true                 *)
...
let bit1_not_even_lemma =       (* |- EVEN (BIT1 n) <=> false    *)
    list_trans_rule
      [ (* |- EVEN (BIT1 n) <=> EVEN (SUC (2 * n))     *)
        mk_comb2_rule (parse_term(@"EVEN")) (spec_rule n bit1_multdef_thm);
        (* |- ...           <=> ~ ODD (SUC (2 * n))    *)
        sym_rule
          (trans_rule (mk_not_rule (spec_rule (parse_term(@"SUC (2*n)")) odd_def))
                      (spec_rule (parse_term(@"EVEN (SUC (2*n))")) not_dneg_thm) );
        (* |- ...           <=> false                  *)
...
let bit0_not_odd_lemma =        (* |- ODD (BIT0 n) <=> false     *)
    list_trans_rule
      [ (* |- ODD (BIT0 n) <=> ~ EVEN (BIT0 n)      *)
        spec_rule (parse_term(@"BIT0 n")) odd_def;
        (* |- ...          <=> false                *)
...
let bit1_odd_lemma =            (* |- ODD (BIT1 n) <=> true     *)
    list_trans_rule
      [ (* |- ODD (BIT1 n) <=> ~ EVEN (BIT1 n)      *)
        spec_rule (parse_term(@"BIT1 n")) odd_def;
        (* |- ...          <=> true                 *)
...
let bit0_eq_zero_lemma =        (* |- BIT0 n = ZERO <=> n = ZERO    *)
  subs_rule [numeral_zero_thm]
   (list_trans_rule
      [ (* |- BIT0 n = 0 <=> 2 * n = 0            *)
        mk_eq1_rule (spec_rule n bit0_multdef_thm) (parse_term(@"0"));
        (* |- ...        <=> 2 = 0 \/ n = 0       *)
        list_spec_rule [(parse_term(@"2"));n] mult_eq_zero_thm;
        (* |- ...        <=> n = 0                *)
        mk_disj1_rule (eqf_intro_rule two_not_zero_thm) (parse_term(@"n=0"));
...
let zero_eq_bit0_lemma =        (* |- ZERO = BIT0 n <=> n = ZERO    *)
    trans_rule (sym_conv (parse_term(@"ZERO = BIT0 n"))) bit0_eq_zero_lemma

let bit1_eq_zero_lemma =        (* |- BIT1 n = ZERO <=> false       *)
    trans_rule
      (* |- BIT1 n = 0 <=> SUC (n + n) = 0        *)
      (mk_eq1_rule (spec_rule n bit1_adddef_thm) (parse_term(@"ZERO")))
      (* |-            <=> false                  *)
      (eqf_intro_rule (spec_rule (parse_term(@"n+n")) suc_not_zero_thm0))

let zero_eq_bit1_lemma =        (* |- ZERO = BIT1 n <=> false       *)
    trans_rule (sym_conv (parse_term(@"ZERO = BIT1 n"))) bit1_eq_zero_lemma
...
let bit0_eq_bit0_lemma =        (* |- BIT0 m = BIT0 n <=> m = n   *)
    trans_rule
      ...
      (* |- ...             <=> m = n             *)
      (mp_rule
        (list_spec_rule [(parse_term(@"2"));m;n] mult_eq_cancel_thm)
        two_not_zero_thm)

let bit0_eq_bit1_lemma =        (* |- BIT0 m = BIT1 n <=> false   *)
    eqf_intro_rule
      (not_intro_rule
        (disch_rule (parse_term(@"BIT0 m = BIT1 n"))
          (* BIT0 m = BIT1 n |- false                *)
          (eq_mp_rule
            (* |- EVEN (SUC (2 * n)) <=> false         *)
            (eqf_intro_rule
              (eq_mp_rule
                (spec_rule (parse_term(@"SUC (2*n)")) odd_def)
                (spec_rule n suc_twice_odd_thm) ))
            (eq_mp_rule
              (* BIT0 m = BIT1 n |- EVEN (2 * m) <=> EVEN (SUC (2 * n))    *)
              (mk_comb2_rule (parse_term(@"EVEN"))
                (* BIT0 m = BIT1 n |- 2 * m = SUC (2 * n)                    *)
                (eq_mp_rule
                  (mk_eq_rule (spec_rule m bit0_multdef_thm)
                              (spec_rule n bit1_multdef_thm) )
                  (assume_rule (parse_term(@"BIT0 m = BIT1 n"))) ))
              (* |- EVEN (2 * m)                                           *)
              (spec_rule m twice_even_thm) ))))

let bit1_eq_bit0_lemma =        (* |- BIT0 m = BIT1 n <=> false   *)
    trans_rule
      (sym_conv (parse_term(@"BIT1 m = BIT0 n")))
    ...
    list_trans_rule
      [ (* |- BIT1 m = BIT1 n <=> SUC (2 * m) = SUC (2 * n)  *)
        mk_eq_rule (spec_rule m bit1_multdef_thm)
                   (spec_rule n bit1_multdef_thm);
        (* |- ...             <=> 2 * m = 2 * n              *)
        list_spec_rule [(parse_term(@"2*m"));(parse_term(@"2*n"))] suc_injective_thm;
        (* |- ...             <=> m = n                      *)
        mp_rule
          (list_spec_rule [(parse_term(@"2"));m;n] mult_eq_cancel_thm)
          two_not_zero_thm ]
...
let rec eval_nat_eq_conv0 tm1 tm2 =
  if (tm1 = tm2)
     then (*  n = n  -->  true                    *)
      ...
      |  (Bit0 tm01, Bit0 tm02)
           -> (*  BIT0 m = BIT0 n  -->  m = n         *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit0_eq_bit0_lemma)
      ...
      | (Bit1 tm01, Bit1 tm02)
           -> (*  BIT1 m = BIT1 n  -->  m = n         *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit1_eq_bit1_lemma)
                         (eval_nat_eq_conv0 tm01 tm02)
      | (Bit0 tm01, Zero)
           -> (*  BIT0 n = ZERO  -->  n = ZERO        *)
              trans_rule (inst_rule [(n,tm01)] bit0_eq_zero_lemma)
                         (eval_nat_eq_conv0 tm01 (parse_term(@"ZERO")))
      | (Bit1 tm01, Zero)
           -> (*  BIT1 n = ZERO  -->  false           *)
              inst_rule [(n,tm01)] bit1_eq_zero_lemma
      | (Zero, Bit0 tm02)
           -> (*  ZERO = BIT0 n  -->  n = ZERO        *)
              trans_rule (inst_rule [(n,tm02)] zero_eq_bit0_lemma)
                         (eval_nat_eq_conv0 (parse_term(@"ZERO")) tm02)
      | (Zero, Bit1 tm01)
           -> (*  ZERO = BIT1 n  -->  false           *)
...
let bit0_le_bit0_lemma =        (* |- BIT0 m <= BIT0 n <=> m <= n  *)
    trans_rule
      (* |- BIT0 m <= BIT0 n <=> 2 * m <= 2 * n   *)
      (mk_bin_rule (parse_term(@"$<=")) (spec_rule m bit0_multdef_thm)
                         (spec_rule n bit0_multdef_thm))
      (* |- ...             <=> m <= n            *)
      (mp_rule
        (list_spec_rule [(parse_term(@"2"));m;n] mult_le_cancel_thm)
        two_not_zero_thm)

let bit1_le_bit1_lemma =        (* |- BIT1 m <= BIT1 n <=> m <= n   *)
    list_trans_rule
      [ (* |- BIT1 m <= BIT1 n <=> SUC (2 * m) <= SUC (2 * n)  *)
        mk_bin_rule (parse_term(@"$<=")) (spec_rule m bit1_multdef_thm)
                          (spec_rule n bit1_multdef_thm);
        (* |- ...              <=> 2 * m <= 2 * n              *)
        list_spec_rule [(parse_term(@"2*m"));(parse_term(@"2*n"))] suc_le_cancel_thm;
        (* |- ...              <=> m <= n                      *)
        mp_rule
          (list_spec_rule [(parse_term(@"2"));m;n] mult_le_cancel_thm)
          two_not_zero_thm ]
    ...
    list_trans_rule
      [ (* |- BIT1 m <= BIT0 n <=> SUC (2 * m) <= 2 * n    *)
        mk_bin_rule (parse_term(@"$<=")) (spec_rule m bit1_multdef_thm)
                          (spec_rule n bit0_multdef_thm);
        (* |- ...              <=> 2 * m < 2 * n           *)
        list_spec_rule [(parse_term(@"2*m"));(parse_term(@"2*n"))] suc_le_lt_thm;
        (* |- ...              <=> m < n                   *)
        mp_rule (list_spec_rule [(parse_term(@"2"));m;n] mult_lt_cancel_thm)
                two_not_zero_thm ]

let bit0_le_bit1_lemma =       (* |- BIT0 m <= BIT1 n <=> m <= n    *)
    list_trans_rule
      [ (* |- BIT0 m <= BIT1 n <=> BIT0 m < BIT1 n \/ BIT0 m = BIT1 n   *)
        list_spec_rule [(parse_term(@"BIT0 m"));(parse_term(@"BIT1 n"))] le_cases_thm;
        (* |- ...              <=> BIT0 m < BIT1 n                      *)
        mk_disj2_rule (parse_term(@"BIT0 m < BIT1 n")) bit0_eq_bit1_lemma;
        spec_rule (parse_term(@"BIT0 m < BIT1 n")) disj_id_thm;
        (* |- ...              <=> 2 * m < SUC (2 * n)                  *)
        mk_bin_rule (parse_term(@"$<")) (spec_rule m bit0_multdef_thm)
                         (spec_rule n bit1_multdef_thm);
        (* |- ...              <=> 2 * m <= 2 * n                       *)
        list_spec_rule [(parse_term(@"2*m"));(parse_term(@"2*n"))] lt_suc_le_thm;
        (* |- ...              <=> m <= n                               *)
        mp_rule (list_spec_rule [(parse_term(@"2"));m;n] mult_le_cancel_thm)
                two_not_zero_thm ]
...
let bit0_lt_bit0_lemma =         (* |- BIT0 m < BIT0 n <=> m < n    *)
    trans_rule
      ...
      (mk_bin_rule (parse_term(@"$<")) (spec_rule m bit0_multdef_thm)
                        (spec_rule n bit0_multdef_thm))
      (mp_rule
        (list_spec_rule [(parse_term(@"2"));m;n] mult_lt_cancel_thm)
        two_not_zero_thm)
    ...
    list_trans_rule
      [ (* |- BIT1 m < BIT0 n <=> 2 * m + 1 < 2 * n        *)
        mk_bin_rule (parse_term(@"$<")) (spec_rule m bit1_multadddef_thm)
                         (spec_rule n bit0_multdef_thm);
        (* |- ...             <=> 2 * m + 2 <= 2 * n       *)
        sym_rule (list_spec_rule [(parse_term(@"2*m+1"));(parse_term(@"2*n"))] suc_le_lt_thm);
        mk_bin1_rule (parse_term(@"$<="))
          (trans_rule
            (sym_rule (list_spec_rule [(parse_term(@"2*m"));(parse_term(@"1"))] add_dist_right_suc_thm))
            (mk_bin2_rule (parse_term(@"$+")) (parse_term(@"2*m")) suc_one_thm) )
          (parse_term(@"2*n"));
        (* |- ...             <=> 2 * (m + 1) <= 2 * n     *)
        mk_bin1_rule (parse_term(@"$<="))
          (trans_rule
            (sym_rule (mk_bin2_rule (parse_term(@"$+")) (parse_term(@"2*m")) (spec_rule (parse_term(@"2")) mult_id_thm)))
            (sym_rule (list_spec_rule [(parse_term(@"2"));m;(parse_term(@"1"))] mult_dist_right_add_thm)) )
          (parse_term(@"2*n"));
        (* |- ...             <=> m + 1 <= n               *)
        mp_rule (list_spec_rule [(parse_term(@"2"));(parse_term(@"m+1"));n] mult_le_cancel_thm)
                two_not_zero_thm;
        (* |- ...             <=> m < n                    *)
        sym_rule (mk_bin1_rule (parse_term(@"$<=")) (spec_rule m suc_add_thm) n);
    ...
    list_trans_rule
      [ (* |- BIT1 m < BIT1 n <=> SUC (2 * m) < SUC (2 * n)    *)
        mk_bin_rule (parse_term(@"$<")) (spec_rule m bit1_multdef_thm)
                         (spec_rule n bit1_multdef_thm);
        (* |- ...             <=> 2 * m < 2 * n                *)
        list_spec_rule [(parse_term(@"2*m"));(parse_term(@"2*n"))] suc_lt_cancel_thm;
        (* |- ...             <=> m < n                        *)
        mp_rule
          (list_spec_rule [(parse_term(@"2"));m;n] mult_lt_cancel_thm)
          two_not_zero_thm ]
...
let rec eval_le_conv0 tm1 tm2 =
  if (tm1 = tm2)
     then (*  n <= n  -->  true                    *)
      ...
      | (Bit0 tm01, Bit0 tm02)
           -> (*  BIT0 m <= BIT0 n  -->  m <= n        *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit0_le_bit0_lemma)
                         (eval_le_conv0 tm01 tm02)
      | (Bit0 tm01, Bit1 tm02)
           -> (*  BIT0 m <= BIT1 y  -->  m <= n        *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit0_le_bit1_lemma)
                         (eval_le_conv0 tm01 tm02)
      | (Bit1 tm01, Bit0 tm02)
           -> (*  BIT1 m <= BIT0 n  -->  m < n         *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit1_le_bit0_lemma)
                         (eval_lt_conv0 tm01 tm02)
      | (Bit1 tm01, Bit1 tm02)
           -> (*  BIT1 m <= BIT1 n  -->  m <= n        *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit1_le_bit1_lemma)
                         (eval_le_conv0 tm01 tm02)
      | (_, Zero)
           -> (*  BITX n <= ZERO  -->  n = ZERO        *)
              trans_rule (inst_rule [(n,tm1)] le_zero_lemma)
                         (eval_nat_eq_conv0 tm1 (parse_term(@"ZERO")))
      | (Zero, _)
           -> (*  ZERO <= n  -->  true                 *)
              inst_rule [(n,tm2)] zero_le_lemma
      | _  -> internal_err "eval_le_conv0"
...
and eval_lt_conv0 tm1 tm2 =
  if (tm1 = tm2)
     then (*  n < n  -->  false                    *)
      ...
      | (Bit0 tm01, Bit0 tm02)
           -> (*  BIT0 m < BIT0 n  -->  m < n          *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit0_lt_bit0_lemma)
                         (eval_lt_conv0 tm01 tm02)
      | (Bit0 tm01, Bit1 tm02)
           -> (*  BIT0 m < BIT1 n  -->  m <= n         *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit0_lt_bit1_lemma)
                         (eval_le_conv0 tm01 tm02)
      | (Bit1 tm01, Bit0 tm02)
           -> (*  BIT1 m < BIT0 n  -->  m < n          *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit1_lt_bit0_lemma)
                         (eval_lt_conv0 tm01 tm02)
      | (Bit1 tm01, Bit1 tm02)
           -> (*  BIT1 m < BIT1 n  -->  m < n          *)
              trans_rule (inst_rule [(m,tm01);(n,tm02)] bit1_lt_bit1_lemma)
                         (eval_lt_conv0 tm01 tm02)
      | (Zero, _)
           -> (*  ZERO < n  -->  ~ (n = ZERO)          *)
              list_trans_rule
                 [ inst_rule [(n,tm2)] zero_lt_lemma;
                   mk_comb2_rule (parse_term(@"$~")) (eval_nat_eq_conv0 tm2 (parse_term(@"ZERO")));
                   not_false_thm ]
      | (_, Zero)
           -> (*  n < ZERO  -->  false                 *)
...
let eval_gt_conv tm =
    ...
    let tm01 = dest_numeral_tag tm1 in
    let tm02 = dest_numeral_tag tm2 in
    let th = trans_rule (inst_rule [(m,tm01);(n,tm02)] gt_lt_lemma)
...
let eval_ge_conv tm =
    ...
    let tm01 = dest_numeral_tag tm1 in
    let tm02 = dest_numeral_tag tm2 in
    let th = trans_rule (inst_rule [(m,tm01);(n,tm02)] ge_le_lemma)
...
let eval_sub_conv0 tm1 tm2 =
    let n1 = dest_bigint_nat0 tm1  
    let n2 = dest_bigint_nat0 tm2 in
    if (eq_big_int n2 ((big_int_of_string "0")))
      then (*  n - ZERO  -->  n                         *)
      ...
      then (* [ z + n = m ]  m - n  -->  z              *)
           let tm3 = mk_bigint_nat0 (sub_big_int n1 n2) in
           mp_rule
