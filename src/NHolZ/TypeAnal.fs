...
(* get promoted to HolErr exceptions in 'infer_pretypes' and 'check_pretype', *)
(* by which stage a user error is certain.                                    *)

let type_err msg = hol_err ("TYPE ERROR", msg)
...
let rec pretype_ok pty =
    ...
    | Ptyvar _ | Ptygvar _
        -> true
    | Ptycomp (x,ptys)
        -> let n = try1 prim_get_tyconst_arity x     ("pretype_ok","?")
           let () = assert1 (eq_big_int (length_big_int ptys) n)
                                   (arity_error (x, n, length_big_int ptys))
           (forall pretype_ok ptys)

let check_pretype pty =
    try
        let _ = pretype_ok pty
        pty
    with HolFail ("TYPE FAIL",msg) -> type_err msg
...
let new_pretype_gvar () =
    let n = !pretype_gvar_counter
    let () = if not (n < System.Int32.MaxValue)
                then hol_err ("PARSER/PRINTER LIMITATION",
...
let generate_const_preterm x =
    let ty = try1 get_const_gtype x        ("generate_const_preterm","?")
    let tyvs = type_tyvars ty
    let pty = type_to_pretype ty
    let foo tyv =
        let x = dest_var_type tyv
        let pty' = new_pretype_gvar ()
        (Ptyvar x, pty')
    let theta = map foo tyvs
    let pty' = pretype_inst theta pty
    Ptmconst (x,pty')
...
let rec detype_preterm0 ptm =
    ...
    | Ptmcomb (ptm1,ptm2)
         -> let ptm1' = detype_preterm0 ptm1
            let ptm2' = detype_preterm0 ptm2
            Ptmcomb (ptm1',ptm2')
    | Ptmabs (v,ptm0)
         -> let v' = detype_preterm0 v
            let ptm0' = detype_preterm0 ptm0
            Ptmabs (v',ptm0')
    | Ptmtyped (ptm0,pty)
         -> let ptm0' = detype_preterm0 ptm0
            Ptmtyped (ptm0',pty)
...
let detype_preterm ptm =
    let () = reset_pretype_gvar_counter ()
    detype_preterm0 ptm
...
let unify_pretypes0 theta0 (pty1,pty2) =
    let theta12 = basic_unify_pretypes (pty1,pty2)
    let theta' = theta_closure theta0 theta12
    theta'
...
let varenv_inst theta (venv:varenv) : varenv =
    let foo ptys = setify (map (pretype_inst theta) ptys)
    snd_map foo venv
...
let close_name0 venv x theta =
    let ptys = assoc x venv
    let (ptys1,ptys2) = partition (is_empty <* pretype_gtyvars) ptys
...
let close_name (x,pty) (theta,venv) =
    let theta' = close_name0 venv x theta
    let pty' = pretype_inst theta' pty
    let venv' = varenv_subtract (varenv_inst theta' venv) (x,pty')
    (theta',venv')

let close_all_names (theta,venv) =
    let xs = setify (map fst venv)
    let theta' = foldr (close_name0 venv) xs theta
    theta'
...
let check_preterm_annotations tm ptm =
    try
        ...
        ptm0'
    with HolErr _ | HolFail _ | Assert_failure _ ->
        internal_err "check_preterm_annotations"
